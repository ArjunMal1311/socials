...
from services.support.path_config import get_eternity_schedule_file_path, get_review_html_path
...
def build_eternity_schedule_html(profile_name: str, verbose: bool = False, status=None) -> Optional[str]:
    ...
    if not os.path.exists(schedule_path):
        log(f"Schedule not found: {schedule_path}", verbose, is_error=True, status=status, log_caller_file="eternity_html.py")
        return None
    ...
    try:
        ...
        log(f"Failed to read schedule file: {e}", verbose, is_error=True, status=status, log_caller_file="eternity_html.py")
        return None
    ...
    out_path = get_review_html_path(profile_name, "eternity")
    try:
        with open(out_path, 'w') as f:
            f.write(html_doc)
        log(f"Generated review HTML: {out_path}", verbose, status=status, log_caller_file="eternity_html.py")
        return out_path
    except Exception as e:
        log(f"Failed to write review HTML: {e}", verbose, is_error=True, status=status, log_caller_file="eternity_html.py")
        return None