...
from services.support.path_config import get_turbin_schedule_file_path, get_review_html_path
...
def _log(message: str, verbose: bool, is_error: bool = False):
    if verbose or is_error:
        ...
        console.print(f"[turbin_html.py] {timestamp}|[{color}]{log_message}[/{color}]")
...
def build_schedule_html(profile_name: str, verbose: bool = False) -> Optional[str]:
    ...
    title = f"Turbin Review - {profile_name}"
...
"""
    ...
    out_path = get_review_html_path(profile_name, "turbin")
    try:
