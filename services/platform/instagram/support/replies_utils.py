...
from services.platform.instagram.support.scout_utils import (
    scout_instagram_reels_comments,
    move_to_next_reel,
    scout_instagram_reels,
)
...
def process_reel_for_action(reel_data: dict, driver, profile_name: str, output_format: str, restrict_filenames: bool, status: Status, verbose: bool, all_replies: list = None) -> dict:
    reel_url = reel_data["reel_url"]
    comments_data = reel_data["comments_data"]

    local_path, cdn_link = None, None
    log(f"Downloading Instagram Reel: {reel_url}...", verbose, log_caller_file="replies_utils.py")
    ...
    if not (local_path and cdn_link):
        log(f"Failed to download reel or get CDN link for {reel_url}", verbose, is_error=True, log_caller_file="replies_utils.py")

    log("Generating automated reply...", verbose, log_caller_file="replies_utils.py")
    generated_reply = generate_instagram_replies(comments_data=comments_data, video_path=local_path, verbose=verbose, profile=profile_name, replies=all_replies)
...
def run_instagram_reply_mode(profile_name: str, count: int, max_comments: int, output_format: str, restrict_filenames: bool, verbose: bool, headless: bool, status: Status):
    all_replies = []
    try:
        ...
        if storage:
            if hasattr(storage, "get_all_approved_and_posted_reels"):
                ...
                log(f"Loaded {len(all_replies)} approved and posted reels for context", verbose, log_caller_file="replies_utils.py")
            else:
                log("Warning: Instagram storage missing get_all_approved_and_posted_reels", verbose, log_caller_file="replies_utils.py")
    except Exception as e:
        log(f"Warning: Could not initialize Instagram storage for context: {e}", verbose, log_caller_file="replies_utils.py")
    ...
    return scout_instagram_reels(
        ...
        headless=headless,
        status=status,
        process_item_callback=lambda data, driver: process_reel_for_action(
            reel_data=data,
            driver=driver,
            profile_name=profile_name,
