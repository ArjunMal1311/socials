...
def _log(message: str, verbose: bool, status=None, is_error: bool = False, api_info: Optional[Dict[str, Any]] = None, api_key_suffix: str = "N/A"):
    if is_error:
        if status:
            status.stop()
        log_message = message
        ...
        color = "bold red"
        console.print(f"[gemini_util.py] {timestamp}|[{color}]{log_message}{quota_str} (Key: {api_key_suffix})[/{color}]")
    elif verbose:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        color = "white"
        console.print(f"[gemini_util.py] {timestamp}|[{color}]{message} (Key: {api_key_suffix})[/{color}]")
        if status:
            status.start()
    elif status:
...
def generate_gemini(media_path: Optional[str], api_key_pool: APIKeyPool, api_call_tracker: APICallTracker, rate_limiter: RateLimiter, prompt_text: str, model_name: str = 'gemini-2.5-flash-lite', status=None, verbose: bool = False):
    uploaded_file = None
    token_count = None
    max_retries = api_key_pool.size() if api_key_pool.size() > 0 else 3
    retries = 0
    ...
    while retries < max_retries:
        try:
            ...
            if not can_call:
                ...
                _log(f"API call blocked: {reason}", verbose, status, is_error=True, api_info=api_info, api_key_suffix=api_key_suffix)
            ...
            if media_path:
                ...
                _log(message, verbose, status, api_key_suffix=api_key_suffix)
                ...
                while time.time() - start_time < timeout_seconds:
                    ...
                    if file_status.state.name == "ACTIVE":
                        ...
                        _log(message, verbose, status, is_error=True, api_key_suffix=api_key_suffix)
            ...
            try:
                ...
                api_info = api_call_tracker.get_quot_info("gemini", "generate", model_name, api_key_suffix)
                _log(f"Gemini Response (no text): {response}", verbose, status, is_error=True, api_info=api_info, api_key_suffix=api_key_suffix)
                ...
                _log(message, verbose, status, is_error=True, api_info=api_info, api_key_suffix=api_key_suffix)
            ...
            api_info = api_call_tracker.get_quot_info("gemini", "generate", model_name, api_key_suffix)
            _log(error_message, verbose, status, is_error=True, api_info=api_info, api_key_suffix=api_key_suffix)
