...
def calculate_x_tokens(profile_name: str, verbose: bool = False, status: Optional[Status] = None) -> Optional[int]:
    clean_log("Calculating tokens for X data...", verbose, status=status)
    ...
    if not communities:
        clean_log(f"No communities specified for X platform in profile '{profile_name}'. Cannot calculate tokens.", verbose, is_error=True, status=status)
        return None
    ...
    for community_name in communities:
        ...
        if not latest_file or not os.path.exists(latest_file):
            clean_log(f"No latest X data file found for profile {profile_name} with prefix '{community_prefix}'. Skipping token calculation for this community.", verbose, is_error=True, status=status)
            continue

        clean_log(f"Loading data from {latest_file} for token calculation (community: {community_name}).", verbose, status=status)
        with open(latest_file, 'r', encoding='utf-8') as f:
            x_data = json.load(f)
        ...
        clean_log(f"Tokens for community '{community_name}': {community_token_count}", verbose, status=status)
