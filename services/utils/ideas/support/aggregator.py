...
def aggregate_platform_data(profile_name: str, verbose: bool = False, status: Optional[Status] = None) -> None:
    _log(f"Aggregating platform data for profile '{profile_name}'...", verbose, status=status)
    ...
    if latest_reddit_file and os.path.exists(latest_reddit_file):
        ...
        _log(f"No Reddit data file found for profile {profile_name}. Skipping Reddit aggregation.", verbose, is_error=True, status=status)
    ...
    if not x_communities:
        _log(f"No X communities specified for profile '{profile_name}'. Skipping X aggregation.", verbose, is_error=True, status=status)
    else:
        for community_name in x_communities:
            ...
            if latest_x_file and os.path.exists(latest_x_file):
                _log(f"Loading X data from {latest_x_file} (community: {community_name})", verbose, status=status)
                ...
                _log(f"No X data file found for community '{community_name}'. Skipping aggregation for this community.", verbose, is_error=True, status=status)

    if not all_items:
        _log("No data to aggregate from any platform.", verbose, is_error=True, status=status)
        return
    ...
    try:
        ...
        _log(f"Aggregated data saved to {output_file_path}", verbose, status=status)
    except Exception as e:
        _log(f"Error saving aggregated data to {output_file_path}: {e}", verbose, is_error=True, status=status)
    
    _log("Platform data aggregation completed.", verbose, status=status)
