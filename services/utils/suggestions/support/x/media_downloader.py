...
def run_media_download(profile_name: str) -> Dict[str, Any]:
    """Run the complete media download workflow for a profile."""
    import json
    ...
    try:
        ...
        updated_approved_filename = f"approved_content_{datetime.now().strftime('%Y%m%d_%H%M%S')}_with_media.json"
