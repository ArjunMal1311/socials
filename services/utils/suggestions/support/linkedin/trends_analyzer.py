...
def aggregate_linkedin_data(profile_name: str, verbose: bool = False) -> Dict[str, Any]:
    scraped_data = []
    try:
        import glob
        ...
        if os.path.exists(suggestions_dir):
            filtered_files = glob.glob(os.path.join(suggestions_dir, "filtered_content_linkedin_*.json"))
            for file_path in filtered_files[-3:]:
                try:
                    with open(file_path, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        posts = data.get('filtered_posts', [])
                        for post in posts:
                            scraped_data.append({
                                'content': post.get('data', {}).get('text', ''),
...
def analyze_trends_with_gemini(data_summary: str, api_key_pool: APIKeyPool, trends_prompt: str = "", verbose: bool = False) -> str:
    if not trends_prompt:
        trends_prompt = "Analyze the following LinkedIn content data and identify all key trends, topics, and keywords that professionals are discussing. Focus on emerging topics, popular keywords, professional sentiment, industry insights, and viral potential. Provide structured analysis with all significant trends, keywords, sentiment insights, and content ideas."
    ...
    prompt_parts.append('{"trends": ["topic1", "topic2", ...], "keywords": ["keyword1", "keyword2", ...], "sentiment": "overall professional sentiment analysis", "content_ideas": ["idea1", "idea2", ...]}')
...
def analyze_linkedin_trends(profile_name: str, verbose: bool = False) -> Dict[str, Any]:
    log(f"Starting LinkedIn trends analysis for profile {profile_name}", verbose, log_caller_file="trends_analyzer.py")

    aggregated_data = aggregate_linkedin_data(profile_name, verbose)
    ...
    for i, post in enumerate(aggregated_data['scraped_posts']):
        data_summary += f"\n{i+1}. [{post['author_name']}] {post['content'][:100]}... (Likes: {post['likes']}, Comments: {post['comments']})"
    ...
    trends_prompt = PROFILES.get(profile_name, {}).get('prompts', {}).get('linkedin_trends', '')
