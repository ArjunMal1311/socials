...
def run_linkedin_content_scheduling(profile_name: str, storage_generated: Optional[BaseStorage] = None, storage_new: Optional[BaseStorage] = None) -> Dict[str, Any]:
    ...
    if storage_generated and storage_new:
        log(f"Fetching approved generated LinkedIn captions from database for {profile_name}", verbose, log_caller_file="scheduling_utils.py")
        ...
        log(f"Fetching approved new LinkedIn content from database for {profile_name}", verbose, log_caller_file="scheduling_utils.py")
                        ...
                        for post in data['generated_posts']:
                            if post.get('generated_caption') and post.get('finalized') is False:
                                approved_content_from_files.append({
                                    'content_id': post.get('data', {}).get('post_id'),
        ...
        new_posts_files = [f for f in os.listdir(suggestions_dir) if f.startswith('new_posts_content_linkedin_') and f.endswith('.json')]
        new_posts_files.sort(reverse=True)

        for file in new_posts_files:
            try:
                with open(os.path.join(suggestions_dir, file), 'r') as f:
                    data = json.load(f)
                    if 'new_posts' in data:
                        for post in data['new_posts']:
                            if post.get('approved') and post.get('scheduled') is not True:
                                approved_content_from_files.append({
                                    'content_id': post.get('id'),
                                    'generated_caption': post.get('text', ''),
                                    'platform': 'linkedin',
                                    'downloaded_media_paths': [],
                                    'type': 'new_content'
                                })
            except Exception as e:
                log(f"Error loading new posts file {file}: {e}", verbose, log_caller_file="scheduling_utils.py")
        ...
        if scheduled_posts:
            ...
            if not (storage_generated and storage_new):
                ...
                if suggestions_file:
                    ...
                    for post in generated_posts:
                        if post.get('type') == 'generated_caption':
                            for gen_post in suggestions_data.get('generated_posts', []):
                                if gen_post.get('data', {}).get('post_id') == post.get('content_id'):
                                    gen_post['finalized'] = True
                        elif post.get('type') == 'new_content':
                            for new_post in suggestions_data.get('new_posts', []):
                                if new_post.get('id') == post.get('content_id'):
                                    new_post['scheduled'] = True
...
def run_linkedin_content_posting(profile_name: str, storage_generated: Optional[BaseStorage] = None, storage_new: Optional[BaseStorage] = None) -> Dict[str, Any]:
    schedule_file = get_schedule_file_path(profile_name)
        ...
        for entry in schedule_data:
            ...
            if post_text:
                ...
                if success:
                    ...
                    if storage_generated and entry.get('type') == 'generated_caption':
                        storage_generated.update_status(content_id=entry['content_id'], status='posted', verbose=verbose)
                    elif storage_new and entry.get('type') == 'new_content':
                        storage_new.update_status(generated_text_id=entry['content_id'], status='posted', additional_updates={'approved': True}, verbose=verbose)
                else:
                    break
